...
class TestFTS5Search:
    ...
    def test_search_projection_skips_context_enrichment_queries(self, db, monkeypatch):
        ...
        statements = []
        read_context = db._read_ctx

        @contextmanager
        def trace_actual_reads():
            # Observe the connection actually borrowed for each read. Opening
            # a separate reader neither observes the pool nor returns its permit.
        ...
        monkeypatch.setattr(db, "_read_ctx", trace_actual_reads)
